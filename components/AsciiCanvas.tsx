...
export const AsciiCanvas: React.FC<AsciiCanvasProps> = ({ options, setOptions, onCapture }) => {
    ...
    const startCamera = async () => {
      try {
        ...
        if (videoRef.current) {
          ...
          // Ensure video actually plays, catching potential AbortErrors during rapid re-renders
          try {
  ...
  useEffect(() => {
    const renderLoop = () => {
      ...
      // We flip horizontally for a natural mirror effect if using front camera
      hiddenCtx.save();
  ...
  return (
    <div className="relative w-full h-full bg-black">
        ...
        <div className="absolute bottom-24 md:bottom-32 left-1/2 transform -translate-x-1/2 flex items-center gap-4 md:gap-8 z-40 w-full justify-center px-4 pb-[env(safe-area-inset-bottom)]">
            {/* Scan & Analyze Button */}
            <button 
                onClick={handleCaptureClick}
                className="bg-black/60 hover:bg-green-900/80 text-green-400 border border-green-500/50 p-4 rounded-full backdrop-blur-xl transition-all active:scale-90 hover:scale-105 shadow-[0_0_20px_rgba(0,0,0,0.5)]"
                title="Scan & Analyze"
            >
                <ScanEye className="w-5 h-5 md:w-6 md:h-6" />
            </button>

            {/* Photo Capture Button (Primary) */}
            <button 
                onClick={handleScreenshotClick}
                className="bg-green-500/20 hover:bg-green-500/40 text-green-400 border border-green-500/50 p-6 md:p-7 rounded-full backdrop-blur-xl transition-all active:scale-90 group relative shadow-[0_0_30px_rgba(0,255,0,0.2)]"
                title="Save Snapshot"
            >
